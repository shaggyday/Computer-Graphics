...
void interpolate(double points[6],int x0,int x1, double MInv[2][2],const double alpha[2],
double betaMinusAlpha[2],double gammaMinusAlpha[2],double st[2]){
    double pTimesBetaMinusAlpha[2],qTimesGammaMinusAlpha[2];
    double pq[2],v[2];
    v[0] = x0-points[0];
    v[1] = x1-points[1];
    ...
    vecAdd3(2,alpha,pTimesBetaMinusAlpha,qTimesGammaMinusAlpha,st);
    int i;
}
...
void triRender(const double a[2], const double b[2], const double c[2], 
    ...
    double betaMinusAlpha[2],gammaMinusAlpha[2];
    int x0, x1;
    ...
    //Case 1: c is left of b, top slope changes
        for (x0=(int)ceil(points[0]); x0<=(int)floor(points[4]);x0=x0+1) {
            for(x1=(int)YCoordOf(x0,slopeLeftRight,points[0],points[1]);x1<=(int)YCoordOf(x0,slopeLeftTop,points[0],points[1]);x1=x1+1){
                interpolate(points,x0,x1,MInv,alpha,betaMinusAlpha,gammaMinusAlpha,st);
                texSample(tex,st[0],st[1],sample);
                pixSetRGB(x0,x1,sample[0]*rgb[0],sample[1]*rgb[1],sample[2]*rgb[2]);
            }
        }
