...
void meshRender(const meshMesh *mesh, const shaShading *sha, 
	...
	if(mesh->attrDim != sha->attrDim){
		...
		for(i = 0;i < mesh->triNum;i = i + 1){
			...
			transformVertex(sha->unifDim,unif,sha->attrDim,a,sha->varyDim,varyA);
			transformVertex(sha->unifDim,unif,sha->attrDim,b,sha->varyDim,varyB);
			transformVertex(sha->unifDim,unif,sha->attrDim,c,sha->varyDim,varyC);
