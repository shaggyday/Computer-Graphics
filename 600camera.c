...
void camWorldFromScreenHomogeneous(const camCamera *cam, double width,
	...
	double isom[4][4], invProj[4][4], invView[4][4], temp[4][4];
	isoGetHomogeneous(&cam->isometry, isom);
	if (cam->projectionType == camORTHOGRAPHIC)
		camGetInverseOrthographic(cam, invProj);
	else
		camGetInversePerspective(cam,invProj);
	mat44InverseViewport(width, height, invView);
	mat444Multiply(isom, invProj, temp);
	mat444Multiply(temp, invView, homog);
}