...
only, we prepend the const qualifier to the array declaration. */

/* To practice this concept, edit the cumulativeSum function above, replacing 
'double a[]' with 'const double a[]'. Then recompile. Did the compiler give you 
an error or warning? (Not all compilers do.) */
...
int integerMultipleIndices(int length, const int a[], int divisor, 
		int indices[]) {
	int i, count = 0;
	for (i = 0; i < length; i += 1)
		if (a[i] % divisor == 0) {
