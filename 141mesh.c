...
void meshRender(const meshMesh *mesh, depthBuffer *buf, const shaShading *sha, isoIsometry *iso,
	...
	if(mesh->attrDim != sha->attrDim){
		...
		for(i = 0;i < mesh->triNum;i = i + 1){
			...
			sha->transformVertex(iso,sha->unifDim,unif,sha->attrDim,a,sha->varyDim,varyA);
			sha->transformVertex(iso,sha->unifDim,unif,sha->attrDim,b,sha->varyDim,varyB);
			sha->transformVertex(iso,sha->unifDim,unif,sha->attrDim,c,sha->varyDim,varyC);
