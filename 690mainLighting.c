...
/* On macOS, compile with...
    clang 690mainLighting.c 000pixel.o -lglfw -framework OpenGL
*/
...
double OmniPLight[3] = {0.5, 2.0, 1.0};
double OmniCLight[3] = {1.0, 1.0, 1.0};
double DirectDLight[3] = {-1.0, -1.0, 1.0};
double DirectCLight[3] = {0.0, 0.0, 0.3};
...
void initializeDirect(directLight *direct, double cLight[3], double dLight[3]){
...
int main(void) {
	if (pixInitialize(SCREENWIDTH, SCREENHEIGHT, "Ray Tracing") != 0)
		...
		/* Initialize lighting */
		initializeOmni(&omni, OmniCLight, OmniPLight);
		initializeDirect(&direct, DirectCLight, DirectDLight);
